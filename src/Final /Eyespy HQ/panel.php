...
<center>
<?php
include "functions.php";			// standard EyespyHQ includes
include "database.php";				//
include "websiteEntries.php";		//
include "websiteStatistics.php";	//	
include "websiteMetadata.php";		//

// create a mysqli connector to interact with the database. 
$mysqli = new mysqli("", "", "", "eyespyhq");
if($mysqli->connect_errno) 
	echo "Failed to connect to MySQL: (" . $mysqli->connect_errno . ") " . $mysqli->connect_error;

$pw = $_POST['pw'];
if($_SESSION['isActive'] == false)    // ensure there are no current sessions currently active 
 {
	if(isset($pw))					  // ensure the user HAS entered a password
	{
		if(isClean($pw)) 			  // ensure the password entered is not malicious (isclean(), functions.php)
		{
			$password = $_POST['pw'];// create a short-hand variable name for the password
			$result = $mysqli->query("SELECT * FROM  logon WHERE PASSWORD =  '$password'"); //perform SQL to check the password does actually exist within the database
			if($result->num_rows >= 1) { // the existence of one record is sufficient to authenticate.
				$_SESSION['isActive'] = true; // the session is now active.
			} else {
				$_SESSION['error'] = "Invalid username/password."; // if SQL returned no records, password must be invalid.
				redirect("index.php");								// return user to login page. 	
			}
		} else {
			$_SESSION['error'] = "Incorrect password/password format."; // if isClean() failes, the password is in an incorrect format.
			redirect("index.php");
		}
	} else {
		$_SESSION['error'] = "Password not set."; // if a user tries to acess the main panel with no active session/no password set within the form
		redirect("index.php");
	} 
} 

if($_SESSION['isActive'] == true) // allways ensure there is a valid and active session to allow the user to view the main content panel
{
	echo "Welcome to HQ Corp. <a href='?logout'>Click here to logout.</a>"; // welcome message + logout url-action
	$request = getRequest($_SERVER['REQUEST_URI']); // extract the current action request from the URI (getRequest(), functions.php)
	
	// eyespy HQ currently caters for 4 different action requests:
		// logout,
		// delete website entries (configuration tab)
		// delete categories (configuration tab)
		// reset hitcounters
	// each following statement checks for the existence of the above action requests and acts accordingly. 
	...
	///<summary>
	///Website record fetching via the websiteEntries class. 
	///</summary>
	...
	///<summary>
	///Website statistics fetching from extending the websiteEntries class.
	///</summary>	
...
<!--
	The following HTML displays all the results within the appropriate content panes defined (here, the spry assets)
-->
